try:
    import phonenumbers
except ImportError:  # pragma: no cover - optional dependency for phone validation
    class _FallbackNumberParseException(Exception):
        pass

    class _FallbackPhoneNumbers:
        NumberParseException = _FallbackNumberParseException
        ...
        def is_valid_number(number):
            return bool(number) and len("".join(ch for ch in str(number) if ch.isdigit())) >= 10

        @staticmethod
        def number_type(number):
            return _FallbackPhoneNumbers.PhoneNumberType.MOBILE

    phonenumbers = _FallbackPhoneNumbers()
...
except ImportError:  # pragma: no cover - optional dependency for user agent rotation
    class UserAgent:
        @staticmethod
        def random():
